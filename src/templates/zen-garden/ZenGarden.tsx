...
import s from './zen-garden.module.css';
...
export default function ZenGarden({ products, settings }: TemplateProps) {
  ...
  return (
    <div className={s.root} style={{ '--accent': settings.primary_color } as React.CSSProperties}>
      ...
      <section className={s.hero}>
        <div className={s.heroOrnament}></div>
        <h1 className={s.heroTitle}>{settings.hero_title || 'How may we help you?'}</h1>
      ...
      {isSearching && (
        <section className={s.searchResults}>
          ...
          {searchResults.length === 0 ? (
            <p className={s.noResults}>{settings.no_results_text || 'No results found. Please try a different term.'}</p>
          ) : (
      ...
      {!isSearching && view.kind === 'home' && (
        <>
          ...
          <section className={s.popular}>
            <h2 className={s.sectionTitle}>{settings.popular_label || 'Common Questions'}</h2>
            <div className={s.faqList}>
