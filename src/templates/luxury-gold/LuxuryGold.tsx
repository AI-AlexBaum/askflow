...
import s from './luxury-gold.module.css';
...
export default function LuxuryGold({ products, settings }: TemplateProps) {
  ...
  return (
    <div className={s.root} style={{ '--accent': settings.primary_color } as React.CSSProperties}>
      ...
      <section className={s.hero}>
        <div className={s.heroOrnament}>&mdash;</div>
        <h1 className={s.heroTitle}>{settings.hero_title || 'How may we assist you?'}</h1>
        ...
        <div className={s.searchWrapper}>
          ...
          <input
            ...
            placeholder={settings.search_placeholder || 'Search our knowledge base...'}
            value={search}
      ...
      {isSearching && (
        <section className={s.searchResults}>
          ...
          {searchResults.length === 0 ? (
            <p className={s.noResults}>{settings.no_results_text || 'No results were found. Please refine your inquiry.'}</p>
          ) : (
      ...
      {!isSearching && view.kind === 'home' && (
        <>
          ...
          <section className={s.popular}>
            <h2 className={s.sectionTitle}>{settings.popular_label || 'Frequently Asked'}</h2>
            <div className={s.faqList}>
