...
import s from './warm-earthy.module.css';
...
export default function WarmEarthy({ products, settings }: TemplateProps) {
  ...
  return (
    <div className={s.root} style={{ '--accent': settings.primary_color } as React.CSSProperties}>
      ...
      <section className={s.hero}>
        <div className={s.heroDecor}>{'\u{1F33F}'}</div>
        <h1 className={s.heroTitle}>{settings.hero_title || 'How can we help today?'}</h1>
        ...
        <div className={s.searchWrapper}>
          ...
          <input
            ...
            placeholder={settings.search_placeholder || 'Find answers...'}
            value={search}
      ...
      {isSearching && (
        <section className={s.searchResults}>
          ...
          {searchResults.length === 0 ? (
            <p className={s.noResults}>{settings.no_results_text || 'No answers found. Try a different search term.'}</p>
          ) : (
      ...
      {!isSearching && view.kind === 'home' && (
        <>
          ...
          <section className={s.popular}>
            <h2 className={s.sectionTitle}>
              {settings.popular_label || 'Common Questions'}
            </h2>
            <div className={s.faqList}>
