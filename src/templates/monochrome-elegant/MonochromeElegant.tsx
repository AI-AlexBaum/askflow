...
import s from './monochrome-elegant.module.css';
...
export default function MonochromeElegant({ products, settings }: TemplateProps) {
  ...
  return (
    <div className={s.root} style={{ '--accent': settings.primary_color } as React.CSSProperties}>
      ...
      <section className={s.hero}>
        <p className={s.heroLabel}>Support</p>
        <h1 className={s.heroTitle}>{settings.hero_title || 'Help Centre'}</h1>
        <p className={s.heroTagline}>{settings.tagline}</p>
        <div className={s.searchWrapper}>
          <svg className={s.searchIcon} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      {isSearching && (
        <section className={s.searchResults}>
          ...
          {searchResults.length === 0 ? (
            <p className={s.noResults}>{settings.no_results_text || 'No articles matched your query. Please refine your search.'}</p>
          ) : (
      ...
      {!isSearching && view.kind === 'home' && (
        <>
          ...
          <section className={s.products}>
            <h2 className={s.sectionTitle}>Topics</h2>
          ...
          <section className={s.popular}>
            <h2 className={s.sectionTitle}>{settings.popular_label || 'Popular Articles'}</h2>
            <div className={s.faqList}>
