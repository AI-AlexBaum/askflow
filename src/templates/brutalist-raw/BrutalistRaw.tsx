...
import s from './brutalist-raw.module.css';
...
export default function BrutalistRaw({ products, settings }: TemplateProps) {
  ...
  const sectionLabels = ['SEC_01', 'SEC_02', 'SEC_03', 'SEC_04', 'SEC_05', 'SEC_06'];
  ...
  return (
    <div className={s.root} style={{ '--accent': settings.primary_color } as React.CSSProperties}>
      ...
      <header className={s.header}>
        <button className={s.headerLogo} onClick={goHome} type="button">
          {settings.header_display !== 'name' && (settings.logo_url ? (
            ...
            <span className={s.headerMark}>//</span>
          ))}
      ...
      <section className={s.hero}>
        <div className={s.heroLabel}>HELP CENTER</div>
        <h1 className={s.heroTitle}>{settings.hero_title || 'WHAT DO YOU NEED?'}</h1>
        ...
        <div className={s.searchWrapper}>
          ...
          <input
            ...
            placeholder={settings.search_placeholder || 'SEARCH DATABASE'}
            value={search}
      ...
      {isSearching && (
        <section className={s.searchResults}>
          ...
          {searchResults.length === 0 ? (
            <p className={s.noResults}>{settings.no_results_text || 'NO RECORDS FOUND. REVISE YOUR QUERY.'}</p>
          ) : (
      ...
      {!isSearching && view.kind === 'home' && (
        <>
          ...
          <section className={s.products}>
            <h2 className={s.sectionTitle}>
              <span className={s.sectionLabel}>[DIRECTORY]</span>
              ALL PRODUCTS
            </h2>
                ...
                <div key={product.id} className={s.productCard}>
                  <button className={s.productName} onClick={() => {
                    ...
                    <span className={s.productBadge}>
                      {sectionLabels[i % sectionLabels.length]}
                    </span>
                  </button>
          ...
          <section className={s.popular}>
            <h2 className={s.sectionTitle}>
              <span className={s.sectionLabel}>[FREQUENT]</span>
              {settings.popular_label || 'TOP QUERIES'}
            </h2>
            <div className={s.faqList}>
