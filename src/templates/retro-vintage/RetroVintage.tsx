...
import s from './retro-vintage.module.css';
...
export default function RetroVintage({ products, settings }: TemplateProps) {
  ...
  return (
    <div className={s.root} style={{ '--accent': settings.primary_color } as React.CSSProperties}>
      ...
      <section className={s.hero}>
        <div className={s.heroBadge}>Help Center &bull; EST. 2024</div>
        ...
        <div className={s.searchWrapper}>
          ...
          <input
            ...
            placeholder={settings.search_placeholder || 'Search the archives...'}
            value={search}
      ...
      {isSearching && (
        <section className={s.searchResults}>
          ...
          {searchResults.length === 0 ? (
            <p className={s.noResults}>{settings.no_results_text || 'No records found in the archives.'}</p>
          ) : (
      ...
      {!isSearching && view.kind === 'home' && (
        <>
          ...
          <section className={s.products}>
            <div className={s.sectionLabel}>Products &amp; Services</div>
              ...
              {products.map((product) => (
                <div key={product.id} className={s.productCard}>
                  <button className={s.productName} onClick={() => {
                    ...
                    {product.name}
                    <span className={s.retroBadge}>v2.0</span>
                  </button>
