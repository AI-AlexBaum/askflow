...
import s from './stripe-minimal.module.css';
...
export default function StripeMinimal({ products, settings }: TemplateProps) {
  ...
  // Collect all items for the category view (including subcategories)
    ...
    <div className={s.root} style={{ '--accent': settings.primary_color } as React.CSSProperties}>
      ...
      {isSearching && (
        <section className={s.searchResults}>
          <h2 className={s.searchResultsTitle}>
      ...
      {!isSearching && view.kind === 'home' && (
        <>
              ...
              {products.map((product) => (
                <div key={product.id} className={s.productCard}>
                  <button className={s.productName} onClick={() => {
                    ...
                    {product.name}
                    <span className={s.productArrow}>&rarr;</span>
                  </button>
          ...
          <section className={s.popular}>
            <h2 className={s.popularTitle}>{settings.popular_label || 'Popular questions'}</h2>
            <div className={s.faqList}>
