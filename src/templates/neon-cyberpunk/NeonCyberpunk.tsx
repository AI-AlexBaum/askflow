...
import s from './neon-cyberpunk.module.css';
...
export default function NeonCyberpunk({ products, settings }: TemplateProps) {
  ...
  return (
    <div className={s.root} style={{ '--accent': settings.primary_color } as React.CSSProperties}>
      ...
      <section className={s.hero}>
        <div className={s.heroEmoji}>&gt;_</div>
        <h1 className={s.heroTitle}>{settings.hero_title || 'SYSTEM // HELP'}</h1>
        ...
        <div className={s.searchWrapper}>
          ...
          <input
            ...
            placeholder={settings.search_placeholder || 'QUERY DATABASE...'}
            value={search}
      ...
      {isSearching && (
        <section className={s.searchResults}>
          ...
          {searchResults.length === 0 ? (
            <p className={s.noResults}>{settings.no_results_text || 'No records found. Refine your query parameters.'}</p>
          ) : (
      ...
      {!isSearching && view.kind === 'home' && (
        <>
                ...
                <div key={product.id} className={s.productCard}>
                  <button className={s.productName} onClick={() => {
                    ...
                    <span className={s.productBadge}>
                      {sectionGlyphs[i % sectionGlyphs.length]}
                    </span>
                  </button>
          ...
          <div className={s.divider}>
            <span className={s.dividerDots}>- - - - -</span>
          </div>
          ...
          <section className={s.popular}>
            <h2 className={s.sectionTitle}>
              <span className={s.sectionEmoji}>//</span>
              {settings.popular_label || 'FREQUENT QUERIES'}
            </h2>
            <div className={s.faqList}>
