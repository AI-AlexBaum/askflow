...
import s from './soft-pastel.module.css';
...
export default function SoftPastel({ products, settings }: TemplateProps) {
  ...
  return (
    <div className={s.root} style={{ '--accent': settings.primary_color } as React.CSSProperties}>
      ...
      <section className={s.hero}>
        <div className={s.heroEmoji}>☁️</div>
        <h1 className={s.heroTitle}>{settings.hero_title || 'Need some help? ☁️'}</h1>
        ...
        <div className={s.searchWrapper}>
          ...
          <input
            ...
            placeholder={settings.search_placeholder || 'Search for answers...'}
            value={search}
      ...
      {isSearching && (
        <section className={s.searchResults}>
          ...
          {searchResults.length === 0 ? (
            <p className={s.noResults}>{settings.no_results_text || 'We couldn\'t find a match. Try searching with different words.'}</p>
          ) : (
      ...
      {!isSearching && view.kind === 'home' && (
        <>
          ...
          <section className={s.popular}>
            <h2 className={s.sectionTitle}>
              <span className={s.sectionEmoji}>💫</span>
              {settings.popular_label || 'Popular Questions'}
            </h2>
            <div className={s.faqList}>
